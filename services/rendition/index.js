...
async function encode(show, job) {
  ...
  // Outside the try above on purpose — a failure here is not ffmpeg's verdict
  // on the clip, so it must not leave a .failed marker. But it has to be
  // caught: thrown, it ended the pass, and with it the whole service.
  try {
